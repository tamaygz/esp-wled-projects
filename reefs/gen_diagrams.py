#!/usr/bin/env python
"""
gen_diagrams.py — Reefs project diagram generator.

Generates all diagram types for the reefs project and saves them to
reefs/docs/.

Usage
-----
    # From repo root:
    python reefs/gen_diagrams.py [--type all|schematic|blocks|concept|wiring]

    # Or use the shared CLI runner:
    python tools/gen_diagrams.py reefs [--type all|schematic|blocks|concept|wiring]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate diagrams for the reefs project.")
    ...
    print(f"\n→ Generating reefs diagrams: {', '.join(types)}\n")
    run_all(types)
    print(f"\n✓ Done — outputs in reefs/docs/")
