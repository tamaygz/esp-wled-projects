...
The YAPP library lives **once** at the repo root in `tools/yapp/YAPPgenerator_v3.scad`
and is shared by every project. Each project provides only its own config file:

| File | Role | Source |
|------|------|--------|
| `tools/yapp/YAPPgenerator_v3.scad` | Library (do not edit) | Pinned v3.3.8 from GitHub |
| `<project>/mechanical/enclosure/<stem>-enclosure.scad` | Project config | Generated by this skill |

From a project enclosure dir, include the library with a relative path:
```scad
...
These are mistakes that have already cost render cycles — do not repeat them.

1. **`include` must be the FIRST statement.** OpenSCAD uses "last assignment wins";
   if `include` is at the bottom, the library's defaults overwrite your project
   values. Put the include on line 1 (after the file-header comment).
2. **Always end the file with an explicit `YAPPgenerate();` call.** The library
   wraps generation in `if (debug)` and will produce an empty STL otherwise.
3. **Use `yappBaseOnly` for PCB standoffs, not `yappBoth`.** The board lives in
   the base only; `yappBoth` adds mirrored stems on the lid that look like
   floating posts and serve no purpose. (`yappBoth` is appropriate only for
   features genuinely shared by both halves.)
4. **Default to `snapJoins`, not `connectors`.** Snap-on lids are the project
   default. Only use screw connectors when the user explicitly asks for them.
5. **`ridgeHeight >= wallThickness * 1.8` whenever `snapJoins` is non-empty.**
   YAPP asserts this. With `wallThickness=3.0` use `ridgeHeight >= 5.4`
   (default to 6.0). Picking 5.0 will fail with a cryptic OpenSCAD error.
6. **Save SCAD as UTF-8 without BOM.** OpenSCAD's parser chokes on the BOM
   marker. In PowerShell:
   ```powershell
   [System.IO.File]::WriteAllText($path, $content, (New-Object System.Text.UTF8Encoding $false))
   ```
7. **Use the helper script for rendering.** `tools/render_enclosure.ps1` produces
   STLs **and** 2 preview PNGs per shell in one call — always commit the previews.
8. **PCB array v3 format requires the leading `"Main"` string** as the first element.
...
Target: 150×100×60mm outer (144×94×56mm inner), 3mm walls, **snap-on lid**.

```scad
// reefs-enclosure.scad — generated 2026-05-16
// Render: pwsh tools/render_enclosure.ps1 -Project reefs -ScadName reefs-enclosure

// Include FIRST so our values below override library defaults
include <../../../tools/yapp/YAPPgenerator_v3.scad>
...
lidWallHeight       = 23;
ridgeHeight         = 6.0;   // >= wallThickness * 1.8 for snapJoins
ridgeSlack          = 0.3;
roundRadius         = 3.0;
boxType             = 0;
...
// Explicit generation call (bypasses library's if(debug) guard)
YAPPgenerate();
```
